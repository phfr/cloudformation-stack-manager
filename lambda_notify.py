...
def lambda_handler(event, context):
    ...
    logger.info("Message: " + str(cf_message))


    # ignore messages that do not pertain to the Stack as a whole (except create_complete)
    if not cf_message['ResourceType'] == 'AWS::CloudFormation::Stack': 
        if cf_message['ResourceStatus'] == 'CREATE_COMPLETE' or cf_message['ResourceStatus'] == 'DELETE_COMPLETE':
            message = {
                'icon_emoji': ':cloud:',
                'username': 'cf-bot',
                'text': '{stack} {event}: {stackres} ({stacktype}) '.format(
                        stack=cf_message['StackName'], event=cf_message['ResourceStatus'], stacktype=cf_message['ResourceType'], stackres=cf_message['LogicalResourceId']),
            }
        else:
            return
    else:
...
def get_stack_update_message(cf_message):
    ...
    # custom, report outputs (dashboards, stream endpoints etc)
 #   if cf_message['LogicalResourceId'] == 'livestreaming' and cf_message['ResourceStatus'] == 'CREATE_COMPLETE':
 #       attachments.append(get_stack_outputs(cf_message['StackName']))
...
#        'fallback': title,
#        'title': title,
#        'fields': [{'title': 'Type {}'.format(output["OutputKey"]), 'value': 'Total {}'.format(output["OutputValue"])}
#                   for output in outputs.iteritems()]
#    }
